...
namespace Yeast.Utilities.Email
{
	public static class MailSender
	{
		public static bool SendMail(string subject, string body, params string[] toMails)
		{
			try
			{
				...
				mailMsg.From = new MailAddress("Info@hemersintoos.ir", "hemersintoos", Encoding.UTF8);
				mailMsg.Sender = new MailAddress("Info@hemersintoos.ir", "hemersintoos", Encoding.UTF8);
				foreach (var mail in toMails)
				{
					mailMsg.To.Add(new MailAddress(mail));
				}
				var smtp = new SmtpClient("hemersintoos.ir", 25);
				smtp.DeliveryMethod = SmtpDeliveryMethod.Network;
				//smtp.EnableSsl = true;
				smtp.Credentials = new NetworkCredential("Info@hemersintoos.ir", "2703Malek");
				smtp.Send(mailMsg);
				return true;
			}
			catch
